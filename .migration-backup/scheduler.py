...
async def reset_mm_free_sessions():
    """Reset free MM sessions at midnight UTC."""
    try:
        import mm_database as mmdb
        mmdb.reset_free_sessions()
        logger.info("✅ MM free session counters reset.")
    except Exception as e:
        logger.error(f"MM reset error: {e}")
...
async def scheduler_loop(bot: Bot):
    ...
    while True:
        try:
            ...
            # Reset MM free sessions once per day at midnight UTC
            from datetime import timezone
            today_utc = datetime.now(timezone.utc).strftime("%Y-%m-%d")
            if today_utc != _last_reset_date:
                _last_reset_date = today_utc
                await reset_mm_free_sessions()
