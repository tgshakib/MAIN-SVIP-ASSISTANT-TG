...
def is_mm_payment(package_name: str) -> bool:
    return package_name.startswith("MM Access")

def get_mm_pkg_by_name(name: str):
    return next((p for p in MM_PACKAGES if p["name"] == name), None)
...
async def approve_payment(callback: CallbackQuery, state: FSMContext, bot):
    ...
    # ── MM Access Payment ────────────────────────────────────
    if is_mm_payment(pkg_name):
        mm_pkg = get_mm_pkg_by_name(pkg_name)
        if mm_pkg:
            days = mm_pkg["days"]
            label = mm_pkg["label"]
            if days == 0:
                ...
                duration_str = f"{days // 30}month"
            else:
                duration_str = str(days)
